...
> 2026-09-03 需求修订：本阶段暂缓，近似/消融远程实验不属于当前验收范围。当前实施顺序改为先用阶段 3 的精确 STT 协议在 MMLU-Redux、GSM8K、MATH-500、LongBench 上完成固定小样本测试并报告逐题记录、任务得分（若现有官方 scorer 可用）、失败、分段耗时和显存；LongBench 在未接入官方 scorer 时必须明确报告“仅生成验证”，不得伪造 accuracy。以下内容保留为后续恢复时的规格。
