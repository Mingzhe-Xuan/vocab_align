...
- Job 247 首次监控：服务器先快进记录提交 `d7cc4cf`；作业在 node221 RUNNING 0:59，source 五 shards 约 0.06s 加载、receiver 五 shards 约 3s，离线 `abstract_algebra` cache 打开成功，暂未见 OOM/traceback。下一次连接继续首项有界 pull并只读查看队列、日志与 records 行数，不取消或重提。
- Job 247 第二次监控：RUNNING 2:07，records 从 Job 246 的 5 条 failed 增至 6 条，表明首个样本已成功重试并跨过旧 OOM；无新 bad sample、traceback 或 summary（作业尚未完成）。下一次连接继续首项有界 pull并查看增量行数/终态。
- Job 247 第三次监控：RUNNING 3:55，records 已增至 8 条，即 3 个原 failed 样本成功追加；bad-samples 保持 11,716 bytes，无 traceback/新错误。下一次连接继续首项有界 pull并等待剩余 2 条及 summary。
- Job 247 初步终态：已离队，records 10 行/55,712 bytes（历史 5 failed + 新 5 success），summary 763 bytes；stdout 明确报告 records/summary 保存，stderr 仅模型 shard 与离线 cache 信息、无 traceback。下一次连接首项有界 pull 后只读审计 summary、latest success、provenance/metrics、SHA 与 partial；不重提。
- Job 247 summary/SHA 初审：latest 统计为 5 success/0 failed/0 correct，平均 source/transport/receiver/decode/total 为 59.3623/0.7554/0.06735/0.04576/60.23085s，平均 source/virtual/output 154.4/154.4/2 tokens，平均 peak CUDA 28,121,007,411 bytes；无 partial。records/summary SHA 分别 `5fd22148...`/`5e7a2def...`，stderr/stdout SHA `699e5ce4...`/`a39b5b07...`。服务器无 jq，下一次连接改用只读 grep 提取唯一 code/runtime/artifact/device-map/support/prediction 字段，完成 provenance 审计。
- 首次 grep 复合审计调用异常结束且无 stdout，未取得新证据、无远端写操作。下一次连接首项有界 pull 后只提取 code/artifact/runtime/device-map，随后另一次记录连接再提取 support/prediction，避免长复合命令。
- provenance 分段审计：records 唯一 artifact SHA 为 `1495d522...`、runtime profile 为 `blackwell-cu128`，device-map override 同时出现 source `cpu`/target `auto`；code version 集含 evaluator `d98a85e5...` 与嵌套 artifact/manifest 的既有构建版本 `f433000f...`、`55825e40...`，符合分层 provenance。下一次连接读取 artifact shape/nnz、support 与 predictions 完成字段验收。
- 字段审计续：artifact shape `[131069,151669]`、nnz 2,733,518；5 条 success 的 active/retained mass 均 1.0、dropped mass 0，prediction 均 null，与 summary 0/5 一致。下一次连接首项有界 pull 后读取去重 generation text/token IDs，确认成功记录确有非空生成；准确率不设通过阈值，但结果不得伪造 prediction。
- generation 初审显示成功记录生成 receiver token IDs `[1062,2]`、解码文本 `>`，因此 parser 合理返回 null；输出非空且 EOS 提前结束（实际 2 tokens），没有伪造选项。下一次连接仅计数该精确 generation 形态是否覆盖 5 条 success，之后完成最终验收。
- Job 247 最终验收：精确 generation `[1062,2]`/`>` 与 status success 均计数 5；latest summary、逐题 provenance/metrics/support、SHA 和无 partial 全部通过。accuracy 0 是小样本质量观测，不影响 evaluator 功能验收，也未被隐藏或重写。集群 accounting disabled，资源以逐题 CUDA peak（平均 28.12GB）和观察到的约 5 分钟 wall time记录；不再重提本作业。
- 2026-09-03 09:56 +08:00 连接用途：同步临时验证提交 `fec0eef`，检查 GSM8K、MATH-500、LongBench Qasper 数据缓存；缺失时仅在任务缓存目录下载。随后以 `script/transport/slurm/evaluate_stt_benchmark.sbatch` 分别提交三个 exact STT 小样本作业，并只读监控/收集逐题记录、summary、日志、资源和 SHA。权限判断：连接后首项必须 `git pull --ff-only`；服务器不修改 Git 源码；模型推理全部通过 Slurm，登录节点只做 pull、缓存检查/下载和队列/文件门禁；本轮明确不提交近似或消融任务。
- 2026-09-03 10:00 +08:00 连接结果与重试用途：首次连接的首项 `timeout 30s git pull --ff-only` 无输出超时，未执行后续操作。下一次连接仍以有界 pull 为首项；若失败，则按更新后的 AGENTS.md 运行 `bash net.sh` 后再次有界 pull。同步成功前不检查缓存、不切换源码、不提交 Slurm 作业；同步后才继续上一条已登记的 exact benchmark 验证范围。
- 2026-09-03 10:02 +08:00 第二次连接结果与第三次用途：首项 20 秒 pull 超时，随后 `bash net.sh` 成功开通网络，但 30 秒 pull 重试仍无输出并以失败结束；没有执行其后的分支、缓存或队列检查。下一次连接仍以 30 秒 `git pull --ff-only` 为首项，只在同步成功后确认当前分支/commit；若同一同步阻塞达到第三次，将先复查 `docs/agents/lessons.md` 的网络经验再决定后续，不绕过 pull 门禁。
- 2026-09-03 10:03 +08:00 第三次连接结果与长轮询用途：首项 45 秒 pull 再次无输出超时，未执行后续操作；已按三次失败规则复查并补充 `lessons.md`。下一次连接仍以 `git pull --ff-only` 为首项，但不再施加过短 timeout，而由本地会话每约 30 秒轮询，给 HTTPS 建连/pack 传输充分时间；若成功仅确认分支/commit，若最终明确失败则保留错误并暂停依赖新源码的作业。
- 2026-09-03 10:06 +08:00 长轮询连接结果与分支诊断用途：约 57 秒后 fetch 成功并取得 `origin/validation/exact-benchmark-smokes`，但当前服务器分支因历史分叉被 `--ff-only` 拒绝；未执行后续命令。下一次连接仍先执行 `git pull --ff-only`，随后仅只读输出当前 branch/HEAD/status，以便在本地从精确服务器基线制作兼容快进提交；不在服务器切换分支、合并或修改源码，诊断后退出。
...
- 2026-09-03 12:11 +08:00 反向模型对前置审计用途：本地方向安全提交 `f255d30` 已推送。连接后首项执行 `git pull --ff-only`；随后只读检查集群、磁盘、Python venv、锁定 Qwen3/Mistral tokenizer 与模型 cache、正式 canonical corpus/manifest 和已有 ANN/artifact 路径，判断能否独立构建 Mistral-Nemo→Qwen3 full T 并跑 exact 最小主表。不修改源码、不提交计算作业、不运行近似/消融。
- 2026-09-03 12:53 +08:00 Planner→Thinker 远程验证用途：临时分支提交 `1ba7f2c` 已推送。连接后首项从 GitHub 拉取 `validation/planner-thinker-20260903`；随后只读核验集群、Blackwell Python venv、正式 artifact/数据与新输出路径门禁，先经 Slurm 提交双 CoT 协议 smoke，通过后再提交 MMLU-Redux、GSM8K、MATH-500、LongBench exact 小样本。所有计算只在 Slurm 内执行，不运行近似/消融，不覆盖旧结果。
- 2026-09-03 12:55 +08:00 首次同步结果与兼容基线诊断用途：首项 GitHub fetch 成功，但服务器当前验证历史与本地 main-based 临时分支分叉，`--ff-only` 安全拒绝且未改变工作树。下一次连接先以服务器本地 HEAD 执行只读 `git pull --ff-only . HEAD` 门禁，再读取 branch/HEAD/status；据该精确基线在本地制作内容等价的兼容快进分支，不在服务器切换、合并或修改源码，不提交计算。
