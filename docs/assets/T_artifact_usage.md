...
当前已验收的有向 transport artifact 对应：
...
词表轴。该 artifact 只能用于上述方向；不能通过转置得到反向 artifact，
也不能复用于其他 tokenizer revision。正式运行不得把它 dense 化，因为
...
二者并不相同。严格 loader 应当拒绝这种组合；不得删除或绕过 fingerprint
检查来强行运行。
